...
# rotary emb
# LOAD_MODE=/nas2/lishengping/caiyun_projects/gpt2-chinese/pretrain_t2023_12_05_13_pos0_ner0_prompt0_base/step20000
# position emb
# LOAD_MODE=/nas2/lishengping/caiyun_projects/gpt2-chinese/pretrain_t2023_12_05_12_pos0_ner0_prompt0_base/step20000
...
OUT_DIR=pretrain_t"${current_year}_${current_month}_${current_day}_${current_hour}"_pos"$ADD_POS"_ner"$ADD_NER"_prompt"$ADD_PROMPT"_"$SIZE"/

LOG_NAME="${current_year}_${current_month}_${current_day}_${current_hour}.txt"

# 512: 5118, 1024: 4590, 1536:3259, 2048: 2317, 3072:
if [[ $1 == 'train' ]]; then
    CUDA_VISIBLE_DEVICES=4,5,6,7 python -W ignore -m torch.distributed.launch $DISTRIBUTED_ARGS pretrain_ddp.py  --train_data_path $TRAIN_PATH --eval_data_path $EVAL_PATH --epochs 10  --log_step 2 --output_dir $OUT_DIR  --perrank_batch_size 4 --gradient_accumulation 1 --lr $LR --warmup_steps 1000 --pretrained_model $LOAD_MODE --save_model_interval_steps 1000 --save_model --eval_step 1000 --add_pos $ADD_POS --add_ner $ADD_NER --add_prompt $ADD_PROMPT --fp16  --use_position_emb 2>&1 --max_len 512| tee logs/$SIZE.$LOG_NAME.$1.log
elif [[ $1 == 'eval' ]]; then
    CUDA_VISIBLE_DEVICES=6,7 python -W ignore -m torch.distributed.launch $DISTRIBUTED_ARGS pretrain_ddp.py --eval_data_path $EVAL_PATH --log_step 2 --perrank_batch_size 1 --pretrained_model $LOAD_MODE --add_pos $ADD_POS --add_ner $ADD_NER --add_prompt $ADD_PROMPT --fp16 --eval --use_position_emb --max_len 3072 2>&1 | tee logs/$SIZE.$LOG_NAME.$1.log
