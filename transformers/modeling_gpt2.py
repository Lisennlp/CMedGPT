...
class RotaryEmbedding(torch.nn.Module):
    ...
    def forward(self, x, seq_len=None):
        ...
        if seq_len > self.max_seq_len_cached:
            ...
            ntk_inv_freq = self.ntk * self.inv_freq
...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids):
    ...
    # print(f'q: {q.shape} k: {k.shape} cos: {cos.shape} sin: {sin.shape} half: {rotate_half(q).shape}')
    cos = cos[position_ids]  # [bs, 1, seq_len, dim]
    sin = sin[position_ids]  # [bs, 1, seq_len, dim]
    # print(f'q: {q.shape} cos: {cos.shape}')
...
class Attention(nn.Module):
    ...
    def forward(self, x, layer_past=None, attention_mask=None, head_mask=None):
        ...
        if self.use_rotary_emb:
            ...
            position_ids = torch.arange(0, kv_seq_len, dtype=torch.long)
            # # [10, 12, 300, 64]  bsz n len head_dim
            query, key = apply_rotary_pos_emb(query, key.permute(0, 1, 3, 2), cos, sin, position_ids)
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.use_position_emb = config.use_position_emb
        if config.add_pos:
            self.pos = nn.Embedding(config.add_pos, config.n_embd)
        if config.add_ner:
            self.ner = nn.Embedding(config.add_ner, config.n_embd)
    ...
    def forward(self, input_ids, pos_tags=None, ner_tags=None, past=None, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None):
        ...
        if self.pos is not None:
            pos_embeds = self.pos(pos_tags)
            hidden_states += pos_embeds
        if self.ner is not None:
            ner_embeds = self.ner(ner_tags)
            hidden_states += ner_embeds
...
class GPT2DoubleHeadsModel(GPT2PreTrainedModel):
    ...
    def forward(self, input_ids, past=None, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None,
                mc_token_ids=None, lm_labels=None, mc_labels=None, loss_mask=None):
        transformer_outputs = self.transformer(input_ids,
                                               past=past,
                                               attention_mask=attention_mask,
                                               token_type_ids=token_type_ids,
